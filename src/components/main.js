...
class Language extends React.Component {
	...
	/*初始化渲染执行之后立刻调用一次*/
	componentDidMount() {
		let source = $.get('http://project.ladytop.net/dist/dataLanguages/languages.json');
		source.then(
			value => {
				this.setState({
				loading: true,
...
class List extends React.Component {
	...
	componentDidMount() {
		...
		let source = $.get('http://project.ladytop.net/dist/dataLanguages/' + language + '_' + this.props.currentTab + '.json');
		source.then(
			value => {
		...
		if(dataCon[0]) {
			...
			dataCon.map(function(v, i) {
				...
				let listHtml = <Panel header={v.aHrefText} key={index} style={customPanelStyle}>
					<p><a href={v.aHref} target='_self'>{v.des}</a></p>
					</Panel>;
